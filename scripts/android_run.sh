...
echo "Starting TextLens Android build and launch process..."
...
if [ -z "$SERIAL" ]; then
  ...
  nohup "$EMU_BIN" -avd "$AVD_NAME" -gpu swiftshader_indirect -no-snapshot-load -no-boot-anim >/tmp/textlens_android_emulator.log 2>&1 &
...
echo "Building TextLens debug APK..."
cd "$ANDROID_DIR"
